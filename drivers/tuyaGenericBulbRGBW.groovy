/**
 * IMPORT URL: https://raw.githubusercontent.com/jshimanek/Hubitat/master/drivers/tuyaGenericBulbRGBW.groovy
 *
 * Copyright 2023-2024 Ivar Holand
 *
 * ---------------------------------------------------------------------------
 * MODIFIED (patched fork) by shimanek, 2026-07-02. Changes from the original:
 *   - Decode HTML entities in the local key (&amp; &gt; &quot; &#39;), not just &lt;.
 *   - Never reuse a session key across a reconfigure; only use it for 3.4.
...
metadata {
	definition(name: "tuya Generic RGBW Bulb",
			namespace: "iholand",
			author: "iholand",
			importUrl: "https://raw.githubusercontent.com/jshimanek/Hubitat/master/drivers/tuyaGenericBulbRGBW.groovy",
			singleThreaded: true) {
		capability "Actuator"
		capability "Bulb"
...
def updated() {
	...
	publishLightEffects()   // PATCH: advertise the effect list on save
...
def setEffect(effectnumber) {
	...
	// PATCH: publish the human-readable effect name via the standard attribute
	sendEvent(name: "effectName", value: tuyaEffectNames()[state.effectnumber.toString()] ?: "Effect ${state.effectnumber}")
...
// PATCH: id -> name map for the device's built-in scenes/patterns. Names taken
// from the effect payloads defined in setEffect().
def tuyaEffectNames() {
	return [
		"0":"Good night","1":"Reading","2":"Working","3":"Leisure","4":"Grassland",
		"5":"Dazzling","6":"Flashing (white/red)","7":"Gorgeous","8":"Night Light",
		"20":"Blue Sky","21":"Sunrise","22":"Sunset Glow","23":"Ocean","24":"Sunflower",
		"25":"Forest","26":"Kung Fu","27":"Candlelight","28":"Dream","29":"Mediterranean",
		"30":"French","31":"American","32":"Birthday","33":"Wedding","34":"Christmas",
		"35":"Independence","36":"Diwali","37":"Holi","38":"Victory Day","39":"Easter",
		"40":"Halloween","41":"Soft","42":"Dynamic"
	]
}

// PATCH: populate the standard lightEffects attribute so apps can discover the
// device's patterns. The original driver declared the LightEffects capability
// but never published this attribute.
def publishLightEffects() {
	def json = new groovy.json.JsonBuilder(tuyaEffectNames()).toString()
	sendEvent(name: "lightEffects", value: json)
}
...
// PATCH: Hubitat HTML-escapes special characters in stored text settings. The
// original driver only decoded &lt;, so a local key containing & (stored as
// &amp;), >, " or ' produced the wrong AES key and commands were silently
// ignored by the device. Decode the common entities (&amp; last).
private String decodeKeyEntities(String s) {
...
byte[] getRealLocalKey() {
	String cleaned = decodeKeyEntities(localKey)
	byte[] staticLocalKey = cleaned.getBytes("UTF-8")

	// Update the setting in case entities got substituted in the original input
	device.updateSetting("localKey", [value: cleaned, type: "text"])
...
def _updatedTuya() {
	...
	state.retry = 5
	state.Msgseq = 1
	state.sessionKey = null   // PATCH: never carry a session key across a reconfigure
}
...
Map decodeIncomingFrame(byte[] incomingData, Integer sofIndex=0, byte[] testKey=null, Closure callback=null) {
	...
	// PATCH: read the full 4-byte big-endian length (original read only 1 byte,
	// which misparses responses >= 256 bytes and throws BadPadding on decrypt).
	Integer frameLength = (Byte.toUnsignedInt(incomingData[sofIndex + 12]) << 24) |
	                      (Byte.toUnsignedInt(incomingData[sofIndex + 13]) << 16) |
	                      (Byte.toUnsignedInt(incomingData[sofIndex + 14]) << 8) |
	                      Byte.toUnsignedInt(incomingData[sofIndex + 15])
...
def generate_payload(String command, def data=null, String timestamp=null, byte[] localkey=getRealLocalKey(), String devid=settings.devId, String tuyaVersion=settings.tuyaProtVersion, Short useMsgSequence=null) {
	...
	// PATCH: only protocol 3.4 uses a negotiated session key. Guard against a
	// stale session key (e.g. left over after switching 3.4 -> 3.3) being used
	// to encrypt, which the device cannot decrypt (commands silently ignored).
	if (tuyaVersion == "34" && state.sessionKey != null) {
	...
	// PATCH: write the length as a full 4-byte big-endian field. The original
	// wrote 3 fixed zero bytes + 1 length byte, corrupting frames >= 256 bytes.
	output.write((payload_len >> 24) & 0xFF)
	output.write((payload_len >> 16) & 0xFF)
	output.write((payload_len >> 8) & 0xFF)
	output.write(payload_len & 0xFF)
	...
	if (tuyaVersion == "34") {
		...
		// PATCH: CRC32 is 4 bytes; zero-pad to a full 8 hex chars. The original
		// only handled one missing leading byte and could emit an odd-length or
		// short CRC (e.g. 7 hex chars), which the device rejects -> command lost.
		while (hex_crc.size() < 8) {
			hex_crc = "0" + hex_crc
		}
